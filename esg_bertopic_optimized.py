...
def strict_esg_preprocess(text: str, nlp_model) -> str:
    ...
    if any(pattern in text.lower() for pattern in reversed_patterns):
        ...
        # If more reversed than normal, reverse the entire text
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='esg_topics_optimized', help='Directory for results')
    ...
    parser.add_argument('--min_cluster_size', type=int, default=60, help='Minimum cluster size for HDBSCAN (higher = fewer, denser topics)')
    parser.add_argument('--nr_topics', type=int, default=120, help='Target number of topics after merging')
    ...
    vectorizer_model = CountVectorizer(
        min_df=0.03,  # Increased from 0.02 - stricter filtering
        max_df=0.5,   # Decreased from 0.6 - remove more common terms
        ngram_range=(1, 2),
        stop_words='english'
    )
    ...
    # UMAP: Optimized for better global structure
    umap_model = UMAP(
        n_neighbors=20,      # Increased from 15 - better global structure
        n_components=10,     # Increased from 5 - preserve more information
        min_dist=0.0, 
        metric='cosine', 
        random_state=42
    )
    ...
    # HDBSCAN: Optimized for reduced outliers
    hdbscan_model = HDBSCAN(
        min_cluster_size=args.min_cluster_size,
        min_samples=15,      # Added for more flexible clustering
        metric='euclidean', 
        cluster_selection_method='eom',  # Better outlier handling
        prediction_data=True
    )
    
    logger.info(f"Initializing Optimized BERTopic (min_cluster_size={args.min_cluster_size}, target_topics={args.nr_topics})...")
    topic_model = BERTopic(
        vectorizer_model=vectorizer_model,
        umap_model=umap_model,
        hdbscan_model=hdbscan_model,
        nr_topics=args.nr_topics,  # Force topic merging to target number
        verbose=True,
